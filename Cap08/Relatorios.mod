...
BEGIN
  ...
  Out.Fixed(rf^.saldoAtual, 10, 2); Out.Ln;
  Out.String("  Despesas Mensais ..: R$ ");
  Out.Fixed(rf^.despesasMensais, 10, 2); Out.Ln;
  Out.String("  Lucro Estimado ....: R$ ");
  Out.Fixed(rf^.saldoAtual - rf^.despesasMensais, 10, 2);
  Out.Ln;
...
BEGIN
  ...
  Out.String(": R$ "); Out.Fixed(rv^.totalVendas, 0, 2);
  Out.Ln;
...
BEGIN
  i := 0;
  WHILE (titulo[i] # 0X) & (i < LEN(r^.titulo) - 1) DO
    r^.titulo[i] := titulo[i];
    INC(i);
  END;
  r^.titulo[i] := 0X; (* Finaliza a string *)
